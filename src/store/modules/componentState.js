...
const useComponentStateStore = defineStore('componentState', {
  state: () => {
    return {
      themeVisible: false, // 控制主题设置面板是否开启
      themesEnum: [
        {
          label: "钴蓝",
          name: "default",
  ...
  actions: {
     ...
     * 
     * @param {*} param0 
     * @param {*} flag 标识是否是通过事件触发
     */
    setTheme({ color, name, isGroup, transition }, flag = false) {
      ...
      // 主题
      if (name) {
        const item = this.themesEnum.find((e) => e.name == name);
        if (item) {
          if (!color) {
            color = item.color;
          }
      ...
      Storage.setItem("themeValue", JSON.stringify(theme));
      
      flag && successMsg(`主题切换为：${theme.label}`)
    }
  }
})

export default useComponentStateStore